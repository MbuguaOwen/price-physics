...
def first_hit_label(df: pd.DataFrame, i0: int, horizon: int, up: float, dn: float):
    hi, lo = df["high"].values, df["low"].values
    end = min(len(df)-1, i0 + horizon)
...
def relabel_snapshot(snapshot_csv, out_csv, atr_window=14, tp_mult=2.0, sl_mult=2.0, horizon_bars=60, bars_root=""):
    ...
    for src, sub in grouped:
        ...
        for i in i0:
            ...
            lab, j = first_hit_label(df, i, horizon_bars, up, dn)
...
def main():
    ...
    ap.add_argument("--horizon_bars", type=int, default=60)
    ...
    relabel_snapshot(
        ...
        horizon_bars=args.horizon_bars, bars_root=args.bars_root
    )
